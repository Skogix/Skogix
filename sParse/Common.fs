...
let applyParser fP xP =
    fP
    >>= (fun f -> xP >>= (fun x -> returnParser (f x)))

let (<*>) = applyParser
// lyfter en f(a->b->c) till parsers
let lift2 f aParser bParser = returnParser f <*> aParser <*> bParser

let andThen p1 p2 =
    p1
    >>= (fun p1Result ->
        p2
        >>= (fun p2Result -> returnParser (p1Result, p2Result)))
...
let orElse p1 p2 =
    let label =
        $"%s{getLabel p1} orElse %s{getLabel p2}"
...
let anyOf chars =
    ...
    chars |> List.map parseChar |> choice <?> label
...
let many1 p =
    ...
    >>= (fun head ->
        many p
        >>= (fun tail -> returnParser (head :: tail)))
    <?> label
...
let parseFloat =
    ...
    let resultToFloat (((sign, digits1), dot), digits2) =
        let float =
            sprintf "%s.%s" digits1 digits2 |> float
